...
const WEAPONS = {
  beretta_m9: { name: "Beretta M9", roleId: "1496247283661013132", required: { steel: 5,    aluminum: 3,    plastic: 2,    iron: 5,    coal: 4    } },
  ak47:       { name: "AK47",       roleId: "1496247352166453278", required: { steel: 500,   aluminum: 500,  plastic: 500,  iron: 500,  coal: 500  } },
  m249:       { name: "M249",       roleId: "1496247139334881352", required: { steel: 1000,  aluminum: 1000, plastic: 1000, iron: 1000, coal: 1000 } },
} as const;
type WeaponKey = keyof typeof WEAPONS;
const CRAFT_ROLE_ID    = WEAPONS.beretta_m9.roleId;
const CRAFT_REQUIRED   = { ...WEAPONS.beretta_m9.required };
...
router.post("/manufacture/craft-weapon", async (req, res) => {
  ...
  const { weapon: weaponRaw } = req.body as Record<string, unknown>;
  const weaponKey = (typeof weaponRaw === "string" && weaponRaw in WEAPONS ? weaponRaw : "beretta_m9") as WeaponKey;
  const weaponCfg = WEAPONS[weaponKey];
  const required  = weaponCfg.required;
  const roleId    = weaponCfg.roleId;
  ...
  (Object.entries(required) as [keyof typeof required, number][])
    .forEach(([k, v]) => { if (resources[k] < v) missing.push(k); });

  if (missing.length) {
    res.status(403).json({ error: "insufficient_resources", missing, message: "الموارد غير كافية لتصنيع "+weaponCfg.name }); return;
  }

  // Deduct resources
  await sql`
    UPDATE manufacture_resources SET
      steel    = steel    - ${required.steel},
      aluminum = aluminum - ${required.aluminum},
      plastic  = plastic  - ${required.plastic},
      iron     = iron     - ${required.iron},
      coal     = coal     - ${required.coal},
  ...
  try { roleAssigned = await assignRole(user.id, roleId); }
  ...
  res.json({ ok: true, roleAssigned, weaponCount: Number(updated?.weapon_count ?? 1), weapon: weaponKey, weaponName: weaponCfg.name });
});
