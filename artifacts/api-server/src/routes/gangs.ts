...
router.post("/gangs/:id/members", async (req, res) => {
  ...
  const { memberUsername, role } = req.body as any;
  if (!memberUsername?.trim()) { res.status(400).json({ error: "memberUsername required" }); return; }

  const assignedRole: string = (role === "vp" && (isPres || isAdmin)) ? "vp" : "member";
  ...
  await sql`
    INSERT INTO gang_members (gang_id, user_id, username, display_name, role, joined_at)
    VALUES (${gang.id}, ${prof.user_id}, ${prof.username}, ${prof.display_name}, ${assignedRole}, ${now})
    ON CONFLICT (gang_id, user_id) DO UPDATE SET role = ${assignedRole}
  `;
  const roleLabel = assignedRole === "vp" ? "نائب رئيس" : "عضو";
  await sql`INSERT INTO gang_log (gang_id, action, actor_username, details, logged_at)
    VALUES (${gang.id}, 'add_member', ${user.username}, ${`تمت إضافة @${prof.username} كـ${roleLabel}`}, ${now})`;
  res.json({ ok: true });
});
