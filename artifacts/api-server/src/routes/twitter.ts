...
interface Tweet {
  ...
  fakeLikeCount: number;
  fakeRetweetCount: number;
  likes: string[];
  retweetedBy: string[];
  replyTo: string | null;
...
function rowToTweet(t: any): Tweet {
  return {
    ...
    videoBase64: t.video_base64 ?? null,
    fakeLikeCount: Number(t.fake_like_count || 0),
    fakeRetweetCount: Number(t.fake_retweet_count || 0),
...
async function upsertTweet(t: Tweet): Promise<void> {
  const sql = getSql();
  await sql`
    INSERT INTO tw_tweets (id, author_id, content, image_base64, video_base64, likes, retweeted_by, reply_to, retweet_of, created_at)
    VALUES (${t.id}, ${t.authorId}, ${t.content}, ${t.imageBase64}, ${t.videoBase64 ?? null}, ${JSON.stringify(t.likes) as any}, ${JSON.stringify(t.retweetedBy) as any}, ${t.replyTo}, ${t.retweetOf}, ${t.timestamp})
...
async function enrichTweet(tweet: Tweet, profiles: Record<string, TwProfile>, tweets: Record<string, Tweet>, myId: string) {
  ...
  if (tweet.retweetOf) {
    ...
    if (orig) {
      ...
      original = { ...orig, hasImage: !!orig.imageBase64, hasVideo: !!orig.videoBase64, imageBase64: undefined, videoBase64: undefined, likeCount: orig.likes.length + (orig.fakeLikeCount || 0), retweetCount: orig.retweetedBy.length + (orig.fakeRetweetCount || 0), replyCount };
  ...
  return {
    ...
    hasVideo: !!tweet.videoBase64,
    imageBase64: undefined,
    videoBase64: undefined,
    likeCount: tweet.likes.length + (tweet.fakeLikeCount || 0),
    retweetCount: tweet.retweetedBy.length + (tweet.fakeRetweetCount || 0),
    replyCount,
...
router.get("/twitter/feed", async (req, res) => {
  ...
  const relevantIds = [user.id, ...myProfile.following];
  const sql = getSql();
  const rows = await sql`SELECT * FROM tw_tweets WHERE author_id = ANY(${relevantIds}) AND reply_to IS NULL ORDER BY created_at DESC LIMIT 60`;
  const tweetList = (rows as any[]).map(rowToTweet);
  const allTweets = await getRecentTweets(500);
  const profiles = await getAllProfiles();
...
// ── Explore (all latest tweets, no profile required) ─────────────────────────
router.get("/twitter/explore", async (req, res) => {
  const user = requireUser(req, res); if (!user) return;
  const limit = Math.min(Number(req.query.limit) || 60, 100);
  const sql = getSql();
  const rows = await sql`SELECT * FROM tw_tweets WHERE reply_to IS NULL AND retweet_of IS NULL ORDER BY created_at DESC LIMIT ${limit}`;
  const tweetList = (rows as any[]).map(rowToTweet);
  const allTweets = await getRecentTweets(500);
  const profiles = await getAllProfiles();
  const tweets = await Promise.all(tweetList.map(t => enrichTweet(t, profiles, allTweets, user.id)));
...
router.post("/twitter/tweet", async (req, res) => {
  const user = requireUser(req, res); if (!user) return;
  const { content, imageBase64, videoBase64, replyTo } = req.body as { content?: string; imageBase64?: string; videoBase64?: string; replyTo?: string };
  if (!content && !imageBase64 && !videoBase64) { res.status(400).json({ error: "content or media required" }); return; }
  const myProfile = await getProfileById(user.id);
  if (!myProfile) { res.status(403).json({ error: "setup profile first" }); return; }
  ...
  if (replyTo) {
    ...
    const tweet: Tweet = {
      ...
      content: (content || "").trim(), imageBase64: imageBase64 ?? null, videoBase64: videoBase64 ?? null,
      fakeLikeCount: 0, fakeRetweetCount: 0,
...
router.delete("/twitter/tweet/:id", async (req, res) => {
  const user = requireUser(req, res); if (!user) return;
  const tweet = await getTweetById(req.params.id);
  if (!tweet) { res.status(404).json({ error: "not_found" }); return; }
  if (tweet.authorId !== user.id && user.username !== ADMIN_DISCORD_USERNAME) {
    res.status(403).json({ error: "forbidden" }); return;
  }
  const sql = getSql();
  await sql`DELETE FROM tw_tweets WHERE id = ${req.params.id}`;
  await sql`DELETE FROM tw_tweets WHERE reply_to = ${req.params.id}`;
  res.json({ ok: true });
});

// ── Delete own Twitter account ────────────────────────────────────────────────
router.delete("/twitter/account", async (req, res) => {
  const user = requireUser(req, res); if (!user) return;
  const sql = getSql();
  await sql`DELETE FROM tw_tweets WHERE author_id = ${user.id}`;
  await sql`DELETE FROM tw_profiles WHERE user_id = ${user.id}`;
  await sql`DELETE FROM tw_notifications WHERE to_user_id = ${user.id} OR from_user_id = ${user.id}`;
  res.json({ ok: true });
});

// ── Trending hashtags ─────────────────────────────────────────────────────────
router.get("/twitter/hashtags/trending", async (req, res) => {
  const user = requireUser(req, res); if (!user) return;
  const sql = getSql();
  const rows = await sql`SELECT content FROM tw_tweets WHERE content IS NOT NULL ORDER BY created_at DESC LIMIT 500`;
  const counts: Record<string, number> = {};
  const pattern = /#([\u0600-\u06ff\w]+)/gi;
  for (const row of rows as any[]) {
...
// ── Tweets by hashtag ─────────────────────────────────────────────────────────
router.get("/twitter/hashtag/:tag", async (req, res) => {
  const user = requireUser(req, res); if (!user) return;
  const tag = req.params.tag.replace(/^#/, "").toLowerCase();
  const sql = getSql();
  const rows = await sql`
    SELECT * FROM tw_tweets WHERE lower(content) LIKE ${"%" + "#" + tag + "%"} AND reply_to IS NULL
...
router.post("/twitter/admin/fake-likes/:tweetId", async (req, res) => {
  const user = requireUser(req, res); if (!user) return;
  if (user.username !== ADMIN_DISCORD_USERNAME) { res.status(403).json({ error: "admin only" }); return; }
  const { count } = req.body as { count?: number };
  if (typeof count !== "number" || count < 0) { res.status(400).json({ error: "count must be >= 0" }); return; }
  const sql = getSql();
  await sql`UPDATE tw_tweets SET fake_like_count = ${Math.floor(count)} WHERE id = ${req.params.tweetId}`;
  res.json({ ok: true });
});

// ── Admin: set fake retweets on a tweet ──────────────────────────────────────
router.post("/twitter/admin/fake-retweets/:tweetId", async (req, res) => {
  const user = requireUser(req, res); if (!user) return;
  if (user.username !== ADMIN_DISCORD_USERNAME) { res.status(403).json({ error: "admin only" }); return; }
  const { count } = req.body as { count?: number };
  if (typeof count !== "number" || count < 0) { res.status(400).json({ error: "count must be >= 0" }); return; }
  const sql = getSql();
  await sql`UPDATE tw_tweets SET fake_retweet_count = ${Math.floor(count)} WHERE id = ${req.params.tweetId}`;
  res.json({ ok: true });
});

// ── Admin: delete any tweet ───────────────────────────────────────────────────
router.delete("/twitter/admin/tweet/:id", async (req, res) => {
  const user = requireUser(req, res); if (!user) return;
  if (user.username !== ADMIN_DISCORD_USERNAME) { res.status(403).json({ error: "admin only" }); return; }
  const sql = getSql();
  await sql`DELETE FROM tw_tweets WHERE id = ${req.params.id}`;
