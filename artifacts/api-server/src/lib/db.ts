...
export async function initSchema(): Promise<void> {
  ...
  await query(`ALTER TABLE tw_tweets ADD COLUMN IF NOT EXISTS video_base64 TEXT`);
  await query(`ALTER TABLE tw_tweets ADD COLUMN IF NOT EXISTS fake_like_count INT NOT NULL DEFAULT 0`);
  await query(`ALTER TABLE tw_tweets ADD COLUMN IF NOT EXISTS fake_retweet_count INT NOT NULL DEFAULT 0`);

  // ── Bank transfers (portal-side ledger) ──────────────────────────────────
  await query(`
  ...
  `);
  await query(`CREATE INDEX IF NOT EXISTS idx_bank_transfers_from ON bank_transfers(from_user_id, transferred_at DESC)`);
  await query(`CREATE INDEX IF NOT EXISTS idx_bank_transfers_to   ON bank_transfers(to_user_id,   transferred_at DESC)`);

  // ── Car rentals (portal-side) ─────────────────────────────────────────────
  await query(`
    CREATE TABLE IF NOT EXISTS car_rentals (
      id               SERIAL PRIMARY KEY,
      car_id           TEXT NOT NULL,
      car_name         TEXT NOT NULL DEFAULT '',
      car_image        TEXT NOT NULL DEFAULT '',
      car_price        BIGINT NOT NULL DEFAULT 0,
      renter_user_id   TEXT NOT NULL,
      renter_username  TEXT NOT NULL DEFAULT '',
  ...
  `);
  await query(`CREATE INDEX IF NOT EXISTS idx_car_rentals_user ON car_rentals(renter_user_id, is_active)`);
}
